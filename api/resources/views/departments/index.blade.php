...
@section('page-title')
    School Information System| Departments
@stop
@section('pageScript')
    ...
    <script>
        ...
        $(".deleteClass").click(function(){
            ...
            $("#yes").click(function(){
                ...
                $.get("<?php echo url('schools/department/remove') ?>/"+id1,function(data){
        ...
        $(".addClass").click(function(){
            ...
            modal+= '<span id="myModalLabel" class="h2 modal-title text-center text-info" style="text-align: center">Create Department</span>';
            modal+= '</div>';
            ...
            $(".modal-body").load("<?php echo url("schools/department/create") ?>");
        ...
        $(".editClass").click(function(){
            ...
            modal+= '<div class="modal-header">';
            modal+= '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
            modal+= '<span id="myModalLabel" class="h2 modal-title text-center text-info" style="text-align: center">Update Department</span>';
            modal+= '</div>';
            modal+= '<div class="modal-body">';
            modal+= ' </div>';
            modal+= '</div>';
            ...
            $("#myModal").modal("show");
            $(".modal-body").html("<h3><i class='fa fa-spin fa-spinner '></i><span>loading...</span><h3>");
            $(".modal-body").load("<?php echo url("schools/department/edit") ?>/"+id1);
            $("#myModal").on('hidden.bs.modal',function(){
                $("#myModal").remove();
            })
                    ...
                    @if( \App\Http\Controllers\UserController::checkAccessRights(Auth::user()->id,2) )
                        <li class='has_sub'><a href='javascript:void(0);'>
                            ...
                            <ul>
                                ...
                                <li><a href='{{url('schools/department/list')}}' class='active'><span><i class="fa fa-arrow-right"></i>Departments</span></a></li>
        ...
        <div class="col-md-12">
            <div class="widget">
                <div class="widget-header">
                    <h2>List of registered departments</h2>
                    <div class="additional-btn">
                        <a class="addClass btn btn-blue-1" style="color: #fff" href="#"><i class="fa fa-file-text-o"></i> New  </a>
                        <a class="btn btn-blue-3" style="color: #fff" href="{{url('schools/department/list')}}"><i class="fa fa-th-list"></i> Manage </a>
                        <a class="btn btn-green-3" style="color: #fff" href="{{url('schools/department/reports')}}"><i class="fa fa-bar-chart-o"></i>  Reports </a>
                    </div>
                </div>
                <div class="widget-content">
                    <br>
                    <div class="table-responsive" id="listClasses" name="{{url('schools/department/list')}}">
                                    ...
                                    @if(count($departments) >0 && $departments != null)
                                        @foreach($departments as $de)
                                            <tr>
                                                ...
                                                <td>{{$el->dept_code}}</td>
                                                <td>{{$de->dept_name}}</td>
                                                <td><?php echo $de->descriptions; ?></td>
                                                <td>{{$de->status}}</td>
                                                <td  id="{{$de->dept_name}}" style="align-content: center"> <div class="col-md-12" id="{{$de->id}}">
                                                        <a href="#" title="Add Streams" class="adduser "><i class="fa fa-users text-success"></i> View</a>&nbsp;&nbsp;&nbsp;
                                                    </div></td>
                                                <td id="{{$de->id}}" style="align-content: center" >
                                                    <div class="col-md-6" id="{{$de->id}}">
